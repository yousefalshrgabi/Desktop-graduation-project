...
class FacultyMembersView extends StatefulWidget {
  const FacultyMembersView({super.key});

  @override
  State<FacultyMembersView> createState() => _FacultyMembersViewState();
}

class _FacultyMembersViewState extends State<FacultyMembersView> {
  ...
  Map<String, String> _userFaculties = {}; // لربط الدكتور بكليته من جدول المستخدمين
    ...
    return Row(
      children: [
        Column(
          ...
          children: [
            const Text('إدارة أعضاء هيئة التدريس', style: DesktopTextStyles.heading1),
            const SizedBox(height: DesktopSpacing.xs / 2),
            Text('إضافة وتعديل وحذف أعضاء هيئة التدريس في الأقسام الأكاديمية', style: DesktopTextStyles.caption),
  ...
  Widget _buildFiltersAndActions(BuildContext context) {
    return Column(
      children: [
            ...
            ListenableBuilder(
                ...
                builder: (context, _) {
                  return ElevatedButton.icon(
                    ...
                    label: const Text('استيراد الإكسل', style: TextStyle(color: Colors.white)),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.green[700],
                      padding: const EdgeInsets.symmetric(horizontal: DesktopSpacing.md, vertical: 16),
            ...
            ElevatedButton(
              onPressed: () {
                showDialog(
                    context: context,
                    barrierDismissible: false,
                    builder: (context) => AddFacultyMemberDialog(viewModel: _viewModel));
              },
        ...
        Row(
          children: [
            _buildDropdownFilter('كل الكليات', _selectedFaculty, _availableFaculties, (val) {
              setState(() {
                _selectedFaculty = val;
                _selectedDepartment = null; // 👈 تصفير القسم إجبارياً عند تغيير الكلية
              });
            }),
            const SizedBox(width: 10),
            _buildDropdownFilter('كل الأقسام', _selectedDepartment, _availableDepartments, (val) {
              setState(() => _selectedDepartment = val);
            }),
            const SizedBox(width: 10),
            _buildDropdownFilter(
                'اللقب العلمي', _selectedDegree, _dynamicDegrees, (val) => setState(() => _selectedDegree = val)),
            const SizedBox(width: 10),
            _buildDropdownFilter(
                'الحالة', _selectedStatus, _dynamicStatuses, (val) => setState(() => _selectedStatus = val)),
            const SizedBox(width: 10),
            TextButton(
              onPressed: () {
                setState(() {
              ...
              child: const Text('مسح المرشحات', style: TextStyle(color: DesktopColors.primary)),
            ),
          ],
        ),
      ],
  ...
  Widget _buildDropdownFilter(
      String label, String? selectedValue, List<String> items, void Function(String?) onChanged) {
      ...
      child: ListenableBuilder(
        ...
        builder: (context, child) {
          if (_viewModel.isLoading) {
            return const Center(
                child: Padding(
                    padding: EdgeInsets.all(DesktopSpacing.lg), child: CircularProgressIndicator()));
          }
          ...
          final members = _viewModel.allMembers.where((m) {
            ...
            final matchesSearch = _searchQuery.isEmpty ||
                ...
                m.generalSpecialization.toLowerCase().contains(_searchQuery.toLowerCase());

            // 2. فلتر الكلية (يعرض كل أعضاء الكلية)
            final memberFaculty = _userFaculties[m.userId] ?? '';
            final matchesFaculty = _selectedFaculty == null || memberFaculty == _selectedFaculty;

            // 3. فلتر القسم (مبني على القسم المختار - اختياري)
            final matchesDept = _selectedDepartment == null || m.department == _selectedDepartment;

            // 4. فلتر اللقب العلمي
            final matchesDegree = _selectedDegree == null || m.currentAcademicTitle == _selectedDegree;

            // 5. فلتر الحالة
            final matchesStatus = _selectedStatus == null || m.status == _selectedStatus;

            return matchesSearch && matchesFaculty && matchesDept && matchesDegree && matchesStatus;
          }).toList();
          ...
          return Column(
            ...
            children: [
              CustomDataTable(
                columns: const [
                  DataColumn(label: Text('الرقم الوظيفي', style: DesktopTextStyles.caption)),
                  DataColumn(label: Text('الاسم', style: DesktopTextStyles.caption)),
                  DataColumn(label: Text('القسم', style: DesktopTextStyles.caption)),
                  DataColumn(label: Text('اللقب العلمي', style: DesktopTextStyles.caption)),
                  DataColumn(label: Text('الحالة', style: DesktopTextStyles.caption)),
                  DataColumn(label: Text('إجراءات', style: DesktopTextStyles.caption)),
                ],
                rows: members.map((member) {
                  return DataRow(cells: [
                    DataCell(Text(member.jobNumber.isNotEmpty ? member.jobNumber : '---',
                    ...
                    DataCell(Text(member.currentAcademicTitle, style: DesktopTextStyles.body)),
                            ...
                            if (member.localFilePath.isNotEmpty || member.fileUrl.isNotEmpty) {
                              ...
                              void parseFiles(String jsonString, bool isUrl) {
                                ...
                                if (jsonString.startsWith('{')) {
                                  try {
                                    ...
                                    decoded.forEach((key, val) {
                                      ...
                                      for (int i = 0; i < list.length; i++) {
                                        ...
                                        var existing =
                                            filesInfo.where((f) => f['category'] == key && f['index'] == i).toList();
                                        if (existing.isNotEmpty) {
                                          if (isUrl) {
                                            existing.first['url'] = str;
                                          } else {
                                            existing.first['path'] = str;
                                          }
                                        } else {
                                          filesInfo.add({
                                            'category': key,
                                  ...
                                  try {
                                    List<String> list = jsonString.startsWith('[')
                                    ...
                                    for (int i = 0; i < list.length; i++) {
                                      ...
                                      if (str.isEmpty) continue;
                                      var existing =
                                          filesInfo.where((f) => f['category'] == 'others' && f['index'] == i).toList();
                                      if (existing.isNotEmpty) {
                                        if (isUrl) {
                                          existing.first['url'] = str;
                                        } else {
                                                  ...
                                                  onPressed: () async {
                                                    bool confirm = await showDialog(
                                                            ...
                                                            builder: (c) => AlertDialog(
                                                                    ...
                                                                    content: const Text(
                                                                        'هل أنت متأكد من رغبتك بحذف هذا الملف نهائياً؟'),
                                                                    actions: [
                                                                      TextButton(
                                                    ...
                                                    if (confirm) {
                                                      ...
                                                      if (context.mounted) {
                                                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                                            content: Text(res ? 'تم حذف الملف بنجاح' : 'فشل حذف الملف')));
                                                      }
                                                    }
                                                  },
                                                ),
                        ...
                        itemBuilder: (context) => [
                          const PopupMenuItem(
                            ...
                            child: Row(children: [
                              ...
                              SizedBox(width: 8),
                              Text('عرض الملف المرفق', style: TextStyle(color: Colors.purple))
                            ]),
                          ),
                          const PopupMenuItem(
                            value: 'edit',
  ...
  Widget _buildStatusBadge(String status) {
    ...
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: DesktopSpacing.xs, vertical: DesktopSpacing.xs / 2),
      decoration: BoxDecoration(
