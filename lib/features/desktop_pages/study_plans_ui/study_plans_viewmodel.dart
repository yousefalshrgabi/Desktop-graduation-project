...
class StudyPlansViewModel extends ChangeNotifier {
  ...
  // 👈 دالة للتحقق بأمان من وجود الملف المحلي دون إحداث خطأ/إغلاق للتطبيق
  ...
  List<Map<String, dynamic>> get excelPlans {
    ...
    sorted.sort((a, b) {
      final aLocal = isLocalFileAvailable(a['local_path']);
      final bLocal = isLocalFileAvailable(b['local_path']);
  ...
  Future<void> uploadStudyPlanExcel(String deptId, PlatformFile file) async {
    try {
      ...
      // تنظيف اسم الملف وإضافة معرف فريد لتجنب تعارض الأسماء ومشاكل قفل نظام التشغيل للملفات
      final sanitizedName = file.name.replaceAll(RegExp(r'[#\[\]*?%:]'), '_');
      final uniqueLocalName = '${const Uuid().v4()}_$sanitizedName';
      final localPath = p.join(targetDir.path, uniqueLocalName);
  ...
  Future<void> downloadExcelFile(Map<String, dynamic> excelRecord) async {
    try {
      ...
      bool existsLocally = isLocalFileAvailable(localPath);
  ...
  Future<void> deleteExcelPlan(Map<String, dynamic> excelRecord) async {
    try {
      ...
      if (isLocalFileAvailable(localPath)) {
        try {
  ...
  Future<void> fetchExcelPlansFromCloud() async {
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        // ضمان ألا يكون local_path فارغاً لمنع خطأ القيود في جدول SQLite
