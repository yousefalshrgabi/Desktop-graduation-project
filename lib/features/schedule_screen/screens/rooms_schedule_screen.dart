...
class RoomsScheduleScreen extends StatefulWidget {
  const RoomsScheduleScreen({super.key});
...
class _RoomsScheduleScreenState extends State<RoomsScheduleScreen> {
  ...
  List<TimetableEntry> _filteredData(String? query, List<TimetableEntry> all) {
    ...
    return all.where((e) => e.room.toLowerCase().contains(query.trim().toLowerCase())).toList();
  }
  ...
  Future<void> _exportSchedule(List<TimetableEntry> all) async {
    ...
    try {
      ...
      if (filtered.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('لا توجد بيانات لهذه القاعة')));
        return;
      }
      ...
      for (final day in days) {
        ...
        for (final hour in hours) {
          final entries = filtered.where((e) => e.day == day && e.hour == hour).toList();
          row.add(entries.isEmpty ? '' : entries.map((e) => '${e.subject}\n${e.teachers.join(", ")}\n${e.studentSets.join(", ")}').join('\n---\n'));
        }
        dataRows.add(row);
      }
      ...
      await ExcelExportService().exportTable(
        ...
        title: 'جدول القاعة: $_searchQuery',
        headers: headers,
        dataRows: dataRows,
      );
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<TimetableEntry>>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.isEmpty) return const Center(child: Text('لا توجد بيانات. يرجى رفع ملف الجدول أولاً.'));
          ...
          children: [
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      value: _searchQuery != null && rooms.contains(_searchQuery) ? _searchQuery : null,
                      decoration: const InputDecoration(
                        labelText: 'اختر القاعة',
                        border: OutlineInputBorder(),
                        contentPadding: EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                      ),
                      items: rooms.map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
            ...
            Expanded(
              child: Padding(
                ...
                child: TimetableDataTable(entries: filtered, showTeachers: true, showStudentSets: true, showRoom: false),
              ),
            ),
          ],
        );
