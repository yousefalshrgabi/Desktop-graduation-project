...
class MobileMyScheduleScreen extends StatefulWidget {
  const MobileMyScheduleScreen({super.key});
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (college.isEmpty || userName.isEmpty) {
        setState(() {
          _errorMessage = 'بيانات الكلية أو المستخدم غير مكتملة.';
          _loading = false;
        });
        return;
      }
      ...
      setState(() {
        _errorMessage = 'تعذر تحميل النصاب: $e';
        _loading = false;
      });
    }
  }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          actions: [
            if (!_loading && _myNasab.isNotEmpty)
              _exporting
                  ? const Padding(
                  ...
                  backgroundColor: MaterialStateProperty.resolveWith<Color>(
                    (Set<MaterialState> states) {
                      if (states.contains(MaterialState.selected)) {
                        return DesktopColors.primary.withOpacity(0.1);
                      }
                      return Colors.white;
                    },
                  ),
